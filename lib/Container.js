...
class Container {
  ...
  get (name) {
    ...
    if (this._singletonServices.has(name) === true) {
      return this._singletonServices.get(name).promise
    }

    // TODO: check scopedRequsts if subcontainer

    // do we even have a way to make it?
    if (this._serviceProviderDefinitions.has(name) === false) {
    ...
    const subgraph = this.createMissingObjectGraph(name)

    // check that we have a provider for every planned operation
    const missingProviders = subgraph.nodes().filter((nodeName) => {
      const serviceName = this._objGraphNodeToServiceName(nodeName)
      !this._serviceProviderDefinitions.has(serviceName)
    })

    // TODO we should dedupe the list before printing it
    if (missingProviders.length > 0) {
      return Promise.reject(new Error(`Missing service providers: ${missingProviders}`))
    }
    ...
    // construct an order for creating deferreds and chaining them together
    const promiseCreationOrder = graphlib.alg.topsort(subgraph)

    promiseCreationOrder.forEach((nodeName) => {
      const serviceName = this._objGraphNodeToServiceName(nodeName)
      // All of our dependencies (if any) providers must be resolved/resolving
      // get the deferreds for any dependencies
      const serviceDefinition = this._serviceProviderDefinitions.get(serviceName)
      const upstreamPromises = subgraph.predecessors(nodeName).map((upstreamObjectName) => {
        // Because all upstream dependencies are exist and are resolved we do not need to existence check (lol)
        const upstreamServiceName = this._objGraphNodeToServiceName(upstreamObjectName)
      ...
      const createObjectPromise = () => {
        return Promise.all(upstreamPromises).then((values) => {
    ...
    const requestedServiceDefinition = this._serviceProviderDefinitions.get(name)
    if (requestedServiceDefinition.lifetime === LIFETIME_TRANSIENT) {
      return transientServices.get(name)
    }
    if (requestedServiceDefinition.lifetime === LIFETIME_SINGLETON) {
      return this._singletonServices.get(name)
    }
  ...
  _objGraphNodeToServiceName (graphNodeName) {
