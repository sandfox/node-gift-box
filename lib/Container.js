...
class Container {
  ...
  get (requestedServiceName) {
    ...
    const subgraph = this.createObjectGraph(requestedServiceName)
    ...
    const newObjectNodeHandler = (nodeName) => {
      // NOTE: by the time we get here the dependencies of this serviceName should
      // already be resolved/resolving
      ...
      // the start of an obj graph can be an already resolved singleton lifespan service
      // if we this is the case don't bother doing any more work
      if (serviceDefinition.lifetime === LIFETIME_SINGLETON && this._singletonServices.has(serviceName)) {
        return
      }
    ...
    // construct an order for creating new promises and chaining them together
    const objectCreationOrder = graphlib.alg.topsort(subgraph)

    objectCreationOrder.forEach(newObjectNodeHandler)
  ...
  createObjectGraph (startNode) {
    ...
    // If we have an already created singleton instance in our object graph
    // we don't want to it add it's children to the Object graph
    const shouldVisitNodesChildren = (serviceName) => {
      return this._singletonServices.has(serviceName) === false
    ...
    function visit (node, child) {
      ...
      if (shouldVisitNodesChildren(node)) {
